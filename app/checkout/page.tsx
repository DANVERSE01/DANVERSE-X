...
export default function CheckoutPage() {
  ...
  // Load configuration — use hardcoded defaults (Admin CMS export is optional override)
  useEffect(() => {
    try {
      const savedContent = localStorage.getItem("danverse-content")
      if (savedContent) {
        const content = JSON.parse(savedContent)
        if (content.orderForm) setOrderConfig(content.orderForm)
      }
    } catch {
      // Ignore — fall back to hardcoded defaults above
    }
  }, [])

  // Currency: use browser locale — no server needed on static export
  useEffect(() => {
  ...
  const handleConfirmOrder = () => {
    ...
    // Fire n8n order intake (non-blocking)
    triggerOrderIntake({
      source: "checkout",
      packageName: order.package?.name ?? "Unknown",
      packagePrice: order.package?.price ?? "Unknown",
      addOns: [
        order.modelingAddOn ? `Modeling: ${order.modelingAddOn.name}` : null,
        order.renderPackage ? `Renders: ${order.renderPackage.name}` : null,
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
                    ...
                    <div className="flex items-center gap-4 w-full sm:gap-5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <div className="font-semibold text-white text-base group-hover:text-orange-400 transition-colors sm:text-lg">
                            {option.title}
                          </div>
                          {"price" in option && option.price && (
                            <div className="text-orange-400 font-bold text-sm sm:text-base">{option.price}</div>
                          )}
                        </div>
                      ...
                      <ArrowRight className="h-5 w-5 text-neutral-600 group-hover:text-orange-400 transition-colors flex-shrink-0 sm:h-6 sm:w-6 sm:ml-3" />
                    </div>
                  </Button>
                ))}
              </div>
      ...
      {/* Updated Sticky Footer Button */}
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a] to-transparent sm:p-6">
        <div className="max-w-sm mx-auto sm:max-w-md">
          {stepContent.isSummary && (
            <Button
